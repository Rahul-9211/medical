  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-green-50/30">
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 space-y-12">
        {/* Hero */}
        <section>
          <div className="bg-white rounded-3xl shadow-lg p-8 grid grid-cols-1 lg:grid-cols-3 gap-6 items-center">
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="flex flex-wrap gap-4 text-sm">
                {hospital.rating && <span>⭐ {hospital.rating.value} ({hospital.rating.reviews} reviews)</span>}
                {hospital.patientsRecommended && <span>✅ {hospital.patientsRecommended.percent}% recommended</span>}
              ...
              {hospital.accreditations?.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-2">
                  {hospital.accreditations.map((a: string, i: number) => (
                    <span key={i} className="px-3 py-1 bg-blue-100 text-blue-700 text-xs rounded-full">{a}</span>
                  ))}
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {hospital.topDoctors.list.map((doc: any) => (
                <div key={doc.id} className="bg-white rounded-xl shadow p-5">
                  ...
                  <a href={doc.profileUrl} className="text-blue-600 hover:underline text-sm mt-2 inline-block">View Profile</a>
                </div>
              ))}
            </div>
          </section>
        ...
        {hospital.media && (
          <section>
            ...
            {hospital.media.images?.filter((img: any) => img.visible).length > 0 && (
